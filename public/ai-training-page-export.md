# AI Training & Enablement Page - Export Package

This document contains all the code needed to replicate the AI Training page in a new Lovable project.
...
## File 1: src/pages/AITraining.tsx
    ...
    <div className="animate-in bg-leap-light">
      ...
      <section className="relative bg-leap-black text-leap-white pt-48 pb-32 overflow-hidden">
        <div className="absolute inset-0 z-0">
          ...
          />
          <div className="absolute inset-0 bg-leap-black/25"></div>
        </div>
...
## File 2: CSS Variables (add to src/index.css :root)

```css
/* LeapUX brand colors */
--leap-brand: 27 71% 38%;
--leap-orange: 27 81% 55%;
--leap-red: 14 69% 51%;
--leap-black: 0 0% 0%;
--leap-white: 0 0% 100%;
--leap-light: 0 0% 99%;
...
## File 3: CSS Components (add to src/index.css)
...
## File 4: Tailwind Config Colors (add to tailwind.config.ts)

```ts
// Add to theme.extend.colors:
leap: {
  brand: "hsl(var(--leap-brand))",
  orange: "hsl(var(--leap-orange))",
  red: "hsl(var(--leap-red))",
  black: "hsl(var(--leap-black))",
  white: "hsl(var(--leap-white))",
  light: "hsl(var(--leap-light))",
},
```

---

## File 5: App Router (add route to src/App.tsx)
...
1. **Hero Background Image**: Upload an AI-themed background image to `src/assets/hero-ai-training.jpg`
   - Recommended: Abstract neural network, circuit patterns, or professional team collaboration image
   - The CSS applies grayscale filter and darkening automatically
...
And configure in tailwind.config.ts:
```ts
fontFamily: {
...
- This page uses shadcn/ui Button component
- Form validation uses Zod
- Toast notifications use the useToast hook from shadcn/ui
- All colors use HSL format with CSS custom properties for theming
